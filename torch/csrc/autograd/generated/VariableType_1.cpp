...
Tensor VariableType::_dequantize_per_tensor(const Tensor & self, double scale, int64_t zero_point, ScalarType dtype) {
  RECORD_FUNCTION("_dequantize_per_tensor", std::vector<c10::IValue>({self}), Node::peek_at_next_sequence_nr());
  auto& self_ = unpack(self, "self", 0);
  std::shared_ptr<NotImplemented> grad_fn;
  if (compute_requires_grad( self )) {
    grad_fn = std::shared_ptr<NotImplemented>(new NotImplemented("_dequantize_per_tensor"), deleteNode);
    grad_fn->set_next_edges(collect_next_edges( self ));
  }
  torch::jit::Node* node = nullptr;
  std::shared_ptr<jit::tracer::TracingState> tracer_state;
  if (jit::tracer::isTracing()) {
    tracer_state = jit::tracer::getTracingState();
    at::Symbol op_name;
    op_name = jit::Symbol::fromQualString("aten::_dequantize_per_tensor");
    node = tracer_state->graph->create(op_name, /*num_outputs=*/0);
    jit::tracer::recordSourceLocation(node);
    jit::tracer::addInputs(node, "self", self);
  ...
  auto tmp = ([&]() {
    at::AutoNonVariableTypeMode non_var_type_mode(true);
    return at::_dequantize_per_tensor(self_, scale, zero_point, dtype);
  })();
  auto result = as_variable(std::move(tmp));
  #ifndef NDEBUG
  if (self__storage_saved.has_value())
...
static auto registerer = torch::RegisterOperators()
  ...
  .op(torch::RegisterOperators::options()
    ...
    .impl_unboxedOnlyKernel<Tensor (const Tensor &, double, int64_t, ScalarType), &VariableType::_dequantize_per_tensor>(TensorTypeId::VariableTensorId)
    .aliasAnalysis(c10::AliasAnalysisKind::FROM_SCHEMA))
  .op(torch::RegisterOperators::options()
